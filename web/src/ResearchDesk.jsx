...
const activityNames = { 'intent.classified': ['识别用户意图', 'Classify intent'], 'agent.started': ['Agent 开始规划', 'Agent planning'], 'agent.completed': ['Agent 完成本轮', 'Agent completed'], 'agent.failed': ['Agent 执行失败', 'Agent failed'], 'agent.recovery_required': ['等待恢复选择', 'Recovery required'], 'agent.recovery_resolved': ['恢复问题已处理', 'Recovery resolved'], 'memory.updated': ['长期上下文已更新', 'Context updated'], 'security.credential_redacted': ['疑似凭证已脱敏', 'Credential redacted'], 'valuation.submitted': ['提交正式估值', 'Submit valuation'] }
...
export function WelcomeHints({ t, onExample, onUpload, target, onTargetChange, onTargetSubmit }) {
  return <div className="research-welcome">
    ...
    <div className="research-hints"><div className="research-hint-title"><Icon name="spark" size={17}/>{t('你可以这样开始', 'Things you can ask')}</div>
      ...
      <p>{t('需要外部数据时，先选择联网获取或自行上传；其他确认也会给出选项，末尾可用 Chat 自定义回答。', 'When external data is needed, choose online retrieval or upload first. Other confirmations also offer choices with Chat for a custom response.')}</p>
    </div>
  </div>
}
...
export function ConfirmationCard({ question, t, busy, onAnswer, onFreeText, onDataServices }) {
  ...
  return <section className="research-confirm" aria-label={t('需要确认', 'Confirmation needed')}>
    ...
    <div className="confirmation-actions"><button className="button primary" disabled={busy || !selected || selected === '__chat__'} onClick={() => onAnswer({ question_id: question.question_id, option_id: selected })}>{t('确认', 'Confirm')}<Icon name="arrow" size={16}/></button></div>
  </section>
}
...
export default function ResearchDesk({ language, t, connection, initialCompany = '', initialResearchId = null, refreshToken = 0, onConnect, onDataServices, onConnectionLost, onWizard, onValuation, onBusyChange, onRestoreLanguage, onHistoryChange, onSessionChange }) {
  ...
  useEffect(() => {
    ...
    api(`/api/research-sessions/${id}`).then(applySnapshot).catch(err => { if (alive.current) setError(err.message) })
  }, [id, refreshToken, applySnapshot])
  ...
  const addFiles = list => {
    ...
    if (chosen.some(file => !/\.(pdf|docx|xlsx|csv|json|txt|md)$/i.test(file.name))) { setError(t('支持 PDF、DOCX、XLSX、CSV、JSON、TXT、MD；旧版 XLS 请先另存为 XLSX。', 'Use PDF, DOCX, XLSX, CSV, JSON, TXT or MD. Save legacy XLS as XLSX first.')); return }
    setError(''); setFiles(previous => [...previous, ...chosen])
  }
  const submitValuation = async () => {
    if (!id || busy || lock.current) return
    lock.current = true; setBusy(true); setError('')
    try {
      const accepted = await post(`/api/research-sessions/${id}/valuation`, {})
      onValuation?.(accepted)
    } catch (err) {
      setError(err.message)
      try { applySnapshot(await api(`/api/research-sessions/${id}`)) } catch { /* Keep current snapshot. */ }
    } finally {
  ...
  const onlineTicker = session?.data_source_preference === 'online' && Boolean(session?.draft?.ticker)

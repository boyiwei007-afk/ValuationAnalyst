...
import { api, post, downloadJson, downloadRun } from './api'
...
function DataServicesModal({ t, researchId, status, onClose, onConnect }) {
  ...
  const connect = async event => {
    ...
    try {
      const next = await post(`/api/research-sessions/${researchId}/data-services`, {
        ...(tavily.trim() ? { tavily_api_key: tavily.trim() } : {}),
  ...
  return <Modal title={t('连接数据服务', 'Connect data services')} onClose={busy ? () => {} : onClose}>
    ...
    <div className="data-service-status">
      ...
      <span className={status?.search?.available ? 'ready' : ''}><i/>{status?.search?.available ? t('Tavily 已就绪', 'Tavily ready') : t('Tavily 未连接', 'Tavily not connected')}</span>
    </div>
    <form onSubmit={connect}>
...
export default function App() {
  ...
  {modal === 'connection' && <ConnectionModal t={t} session={session} onClose={closeModal} onConnect={connect} onDisconnect={async () => { try { await api(`/api/model-sessions/${session.session_id}`, { method: 'DELETE' }); setSession(null); setModal(null) } catch (err) { setError(err.message) } }}/>} {modal === 'data-services' && researchId && <DataServicesModal t={t} researchId={researchId} status={dataServices} onClose={closeModal} onConnect={connectDataServices}/>} {modal === 'review' && record && <ReviewModal {...{record,t,submitting}} onClose={closeModal} onSubmit={revise}/>} {modal === 'history' && <Modal title={t('研究历史', 'Study history')} onClose={closeModal} className="wide-modal">{allStudies.length ? <div className="history-table">{allStudies.map(run => <button key={run.run_id || run.session_id} onClick={() => selectStudy(run)} disabled={sending || submitting}><span><b>{studyName(run)}</b><small>{run.session_id ? new Date(run.updated_at).toLocaleDateString() : run.request.valuation_date} · {studyMode(run)} · v{run.revision}</small></span>{run.session_id ? <span className="tag">{run.status === 'waiting_confirmation' ? t('待确认', 'Your choice') : t('资料研究', 'Research')}</span> : <Status status={run.status} t={t}/>}<Icon name="chevron" size={16}/></button>)}</div> : <Empty title={t('还没有研究任务', 'No studies yet')}>{t('创建研究后，记录会自动保留。', 'New studies are saved automatically.')}</Empty>}</Modal>}
  </div>
}

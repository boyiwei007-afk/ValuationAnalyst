...
function ConnectionModal({ t, onClose, onConnect, session, onDisconnect }) {
  const [config, setConfig] = useState({ provider: 'openai_compatible', base_url: session?.base_url || 'https://api.openai.com/v1', model: session?.model || '', api_key: '' })
  ...
  return <Modal title={t('连接研究模型', 'Connect a research model')} onClose={busy ? () => {} : onClose}><p className="modal-intro">{t('Live Agent 使用你配置的模型。演示与结构化估值无需 API Key。', 'Live Agent uses your model. Demo and snapshot valuation require no API key.')}</p>{session && <div className="connected-banner"><Icon name="check" size={17}/><span>{session.model}</span><button type="button" onClick={onDisconnect} disabled={busy}>{t('断开', 'Disconnect')}</button></div>}<form onSubmit={connect}><label className="field">{t('接口类型', 'Provider protocol')}<select value={config.provider} onChange={e => update('provider', e.target.value)}><option value="openai_compatible">OpenAI Compatible</option><option value="openai">OpenAI</option></select></label><label className="field">Base URL<input type="url" required value={config.base_url} onChange={e => update('base_url', e.target.value)}/></label><label className="field">{t('模型名称', 'Model name')}<input required value={config.model} onChange={e => update('model', e.target.value)} placeholder={t('填写供应商提供的模型名称', 'Enter your provider’s model name')} maxLength={200}/></label><label className="field">API Key<input type="password" autoComplete="off" required value={config.api_key} onChange={e => update('api_key', e.target.value)} placeholder="sk-…"/></label><p className="privacy-note"><Icon name="shield" size={15}/>{t('密钥只用于当前后端会话，不写入浏览器存储、任务或报告。连接时会验证工具调用能力。', 'Keys stay in the current backend session, outside browser storage, tasks and reports. Connections verify tool calling.')}</p><ErrorNotice message={error}/><div className="modal-actions"><button className="button secondary" type="button" onClick={onClose} disabled={busy}>{t('取消', 'Cancel')}</button><button className="button primary" disabled={busy}>{busy ? t('正在验证连接…', 'Verifying connection…') : t('验证并连接', 'Verify & connect')}<Icon name="link" size={17}/></button></div></form></Modal>
}
...
export default function App() {
  ...
  const newTask = () => { if (sending || submitting) return; activate(null); setDraft(initialDraft(language)); setDraftVersion(value => value + 1); setView('workspace'); setError('') }
  ...
  const connect = connected => { setSession(connected); setModal(null); setError(''); if (pendingStart.current) { const request = pendingStart.current; pendingStart.current = null; void create(request, connected) } }

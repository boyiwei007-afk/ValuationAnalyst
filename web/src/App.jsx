...
import { api, post, downloadRun } from './api'
...
function ConnectionModal({ t, onClose, onConnect, session, onDisconnect }) {
  ...
  return <Modal title={t('连接研究模型', 'Connect a research model')} onClose={busy ? () => {} : onClose}>
    ...
    <form onSubmit={connect}>
      ...
      <details className="connection-advanced"><summary>{t('高级连接设置（自定义接口时展开）', 'Advanced connection settings')}</summary>
...
function DataServicesModal({ t, researchId, status, onClose, onConnect }) {
  ...
  const connect = async event => {
    ...
    if (!tavily.trim() && !tushare.trim()) {
      setError(t('至少填写一个服务凭证。', 'Enter at least one service credential.'))
      return
    }
    setBusy(true); setError('')
    try {
      const next = await post(`/api/research-sessions/${researchId}/data-services`, {
        ...(tavily.trim() ? { tavily_api_key: tavily.trim(), verify_search: true } : {}),
        ...(tushare.trim() ? { tushare_token: tushare.trim() } : {}),
      })
      setTavily(''); setTushare(''); onConnect(next)
    } catch (err) { setError(err.message) } finally { setBusy(false) }
  }
  return <Modal title={t('连接数据服务', 'Connect data services')} onClose={busy ? () => {} : onClose}>
    <p className="modal-intro">{t('Tushare 用于正式 A 股财务与行情取数；Tavily 用于公开网页检索和来源补充。可以只连接当前需要的服务。', 'Tushare supplies formal A-share financial and market data. Tavily searches public sources. Connect either or both.')}</p>
    <div className="data-service-status">
      <span className={status?.market?.available ? 'ready' : ''}><i/>{status?.market?.available ? t('Tushare 已就绪', 'Tushare ready') : t('Tushare 未连接', 'Tushare not connected')}</span>
      <span className={status?.search?.connection_status === 'verified' ? 'ready' : ''}><i/>{status?.search?.connection_status === 'verified' ? t('Tavily 已验证', 'Tavily verified') : status?.search?.available ? t('Tavily 已配置，尚未验证', 'Tavily configured, unverified') : t('Tavily 未连接', 'Tavily not connected')}</span>
    </div>
    <form onSubmit={connect}>
      <label className="field">Tushare Token<input type="password" autoComplete="off" value={tushare} onChange={event => setTushare(event.target.value)} placeholder={t('用于 A 股结构化取数', 'For structured A-share data')}/></label>
      <label className="field">Tavily API Key<input type="password" autoComplete="off" value={tavily} onChange={event => setTavily(event.target.value)} placeholder={t('用于网页搜索', 'For web search')}/></label>
      <p className="privacy-note"><Icon name="shield" size={15}/>{t('凭证仅保存在当前后端进程的本研究会话中，不写入浏览器存储、数据库、事件或报告；首次实际调用时验证。', 'Credentials remain only in memory for this research session. They are excluded from browser storage, databases, events and reports, and verified on first use.')}</p>
      <ErrorNotice message={error}/><div className="modal-actions"><button className="button secondary" type="button" onClick={onClose} disabled={busy}>{t('取消', 'Cancel')}</button><button className="button primary" disabled={busy || (!tavily.trim() && !tushare.trim())}>{busy ? t('正在连接…', 'Connecting…') : t('连接所填服务', 'Connect services')}<Icon name="link" size={17}/></button></div>
    </form>
  </Modal>
}
...
function Conversation({ record, runId, messages, revisions, pending, sending, loading, t, onSend, onSelect, onReview, onResume, onReport, session, onConnect, wizard }) {
  ...
  return <section className="conversation-panel"><div className="panel-title"><span><Icon name="chat"/>{t('研究对话', 'Research conversation')}</span>{record ? <select className="version-select" aria-label={t('研究版本', 'Study revision')} value={record.run_id} disabled={sending} onChange={e => onSelect(revisions.find(r => r.run_id === e.target.value))}>{revisions.map(r => <option key={r.run_id} value={r.run_id}>v{r.revision} · {statusLabel(r.status, t)}</option>)}</select> : <span className="subtle">ValuationAgent</span>}</div><div className="chat-scroll" ref={scroll} onScroll={() => { const el = scroll.current; nearBottom.current = el.scrollHeight - el.scrollTop - el.clientHeight < 90 }}>
    {!runId ? <><div className="agent-message"><span className="avatar"><Icon name="spark"/></span><div><b>ValuationAgent</b><p>{t('先确定研究对象。接下来，我会协助你确认数据、设置经营假设，并运行估值模型。', 'First, select your research subject. Then we will review data, set assumptions and run the valuation.')}</p></div></div>{wizard}</> : <>{loading && !record && <div className="loading-state"><span className="mini-spinner"/>{t('正在载入研究…', 'Loading study…')}</div>}{record && <div className="conversation-context"><Icon name="folder" size={16}/><span>{record.request.company.name || record.request.company.ticker}</span><small>{record.request.mode.toUpperCase()} · {record.request.valuation_date}</small></div>}{messages.filter(m => m.role === 'user' || m.role === 'assistant').map(message => <div key={message.message_id} className={`agent-message ${message.role === 'user' ? 'user-message' : ''}`}><span className="avatar">{message.role === 'user' ? t('我', 'Me') : <Icon name="spark"/>}</span><div className="message-content"><div className="message-meta"><b>{message.role === 'user' ? t('你', 'You') : 'ValuationAgent'}</b><span>v{message.revision} · {new Date(message.created_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span></div><p>{message.content}</p>{message.related_run_id && <button className="revision-link" onClick={() => onSelect(revisions.find(r => r.run_id === message.related_run_id))}>{t('查看重算版本', 'Open recalculated revision')}<Icon name="arrow" size={14}/></button>}</div></div>)}{pending && !messages.slice(-4).some(m => m.role === 'user' && m.content === pending) && <div className="agent-message user-message"><span className="avatar">{t('我', 'Me')}</span><div><b>{t('你', 'You')}</b><p>{pending}</p></div></div>}{sending && <div className="working-message" role="status"><span className="mini-spinner"/>{t('正在处理请求；执行进度显示在右侧。', 'Processing your request. Follow execution in the workflow panel.')}</div>}
      {record && ['waiting_review','failed'].includes(record.status) && <div className="review-card"><div><Icon name="alert"/><h3>{t('计算未完成，说明报告可下载', 'Calculation incomplete; report available')}</h3></div><p>{(record.review || record.error)?.message}</p><div className="inline-actions"><button className="button primary" onClick={onReport}><Icon name="download" size={16}/>{t('下载 PDF 结果说明', 'Download PDF outcome')}</button><button className="button secondary" onClick={onReview} disabled={sending}>{t('更正数据', 'Correct inputs')}</button><button className="button secondary" onClick={onResume} disabled={sending}><Icon name="refresh" size={16}/>{t('恢复执行', 'Resume')}</button></div></div>}
...
export default function App() {
  ...
  const [researchId, setResearchId] = useState(() => window.location.hash.match(/^#(research_[a-f0-9]+)$/)?.[1] || null)
  const [health, setHealth] = useState('loading')
  const [session, setSession] = useState(null)
  const [dataServices, setDataServices] = useState({ search: { available: false, provider: 'unavailable' }, market: { available: false, provider: 'unavailable' } })
  ...
  const syncRuns = useCallback(async () => {
    try { const [check, items, studies] = await Promise.all([api('/health'), api('/api/runs'), api('/api/research-sessions')]); if (!mounted.current) return; setHealth(check.status === 'ok' ? 'online' : 'offline'); setRuns(items); setResearchStudies(studies) } catch { if (mounted.current) setHealth('offline') }
  }, [])
  ...
  useEffect(() => {
    ...
    api(`/api/research-sessions/${researchId}/data-services`).then(next => { if (active) setDataServices(next) }).catch(() => {})
    return () => { active = false }
  }, [researchId])
  ...
  const activate = id => { setRunId(id); setResearchId(null); window.history.replaceState(null, '', id ? `#${id}` : window.location.pathname); setMobileNav(false) }
  ...
  const selectStudy = study => {
    ...
    if (study.run_id) { selectRun(study); return }
    activate(null); setResearchId(study.session_id); setShowWizard(false); setDraftVersion(value => value + 1)
    setLanguage(study.language); setView('workspace'); setError(''); setModal(null)
    window.history.replaceState(null, '', `#${study.session_id}`)
  }
  const allStudies = [...runs, ...researchStudies].sort((a, b) => String(b.updated_at || b.created_at).localeCompare(String(a.updated_at || a.created_at)))
  const studyName = study => study.session_id ? study.draft.company || study.draft.ticker || study.draft.objective || t('未命名研究', 'Untitled study') : study.request.company.name || study.request.company.ticker || t('未命名研究', 'Untitled study')
  const studyMode = study => study.session_id ? t('资料研究', 'RESEARCH') : study.request.mode.toUpperCase()
  const currentStudy = !runId && researchStudies.find(study => study.session_id === researchId)
  ...
  const exportResult = () => downloadRun(record.run_id, 'json').catch(err => setError(err.message))
  const openValuation = accepted => { activate(accepted.run_id); setView('workspace'); void syncRuns() }
  ...
  const connectDataServices = next => {
    setDataServices(next); setModal(null); setError('')
    if (!researchId || !next.search.available) return
    void (async () => {
      try {
        const current = await api(`/api/research-sessions/${researchId}`)
        const question = current.session?.question
        if (!['search_unavailable', 'search_failed'].includes(question?.kind)) return
        setSending(true)
        await post(`/api/research-sessions/${researchId}/messages`, {
          content: t('Tavily 搜索已经连接，请立即重试刚才的检索请求。', 'Tavily Search is connected. Retry the previous search now.'),
  ...
  const conversation = <Conversation key={runId || `draft-${draftVersion}`} {...{record, runId, t, sending, pending, session, wizard}} messages={record ? research.messages : []} revisions={record ? research.revisions : []} loading={research.loading} onSend={send} onSelect={selectRun} onReview={() => setModal('review')} onResume={resume} onReport={() => downloadRun(record.run_id, 'pdf').catch(err => setError(err.message))} onConnect={() => setModal('connection')}/>
  const activeNav = navigation.find(([id]) => id === view)
  return <div className="app-shell"><aside className="sidebar">{sidebar}</aside>{mobileNav && <div className="mobile-navigation"><button className="mobile-backdrop" onClick={() => setMobileNav(false)} aria-label={t('关闭导航', 'Close navigation')}/><aside className="sidebar mobile-sidebar">{sidebar}</aside></div>}<div className="main-shell"><header className="topbar"><div className="breadcrumbs"><button className="mobile-menu" onClick={() => setMobileNav(true)} aria-label={t('打开导航', 'Open navigation')}><Icon name="menu"/></button>{t('项目空间', 'Project space')}<span>/</span><b>{t(activeNav[2], activeNav[3])}</b></div><div className="top-actions"><span className={`service-state ${health}`} title={t('后端连接状态', 'Backend connection')}><i/>{health === 'online' ? t('服务在线', 'Online') : health === 'offline' ? t('服务未连接', 'Offline') : t('连接中', 'Connecting')}</span><select aria-label={t('界面语言', 'Interface language')} value={language} onChange={e => setLanguage(e.target.value)}><option value="zh-CN">简体中文</option><option value="en-US">English</option></select>{!runId && <button className="button secondary data-button" disabled={!researchId} onClick={() => setModal('data-services')} title={researchId ? t('连接本研究的数据服务', 'Connect data services for this study') : t('研究会话创建后可配置', 'Available after the study starts')}><Icon name="layers"/>{dataServices.market.available || dataServices.search.available ? t('数据已连接', 'Data connected') : t('数据服务', 'Data services')}</button>}<button className="button secondary model-button" onClick={() => setModal('connection')}><Icon name="settings"/>{session ? t('模型已连接', 'Model connected') : t('模型连接', 'Connect model')}</button></div></header><main className="page"><div className="page-heading"><div><div className="eyebrow">RESEARCH WORKSPACE</div><h1>{record ? record.request.company.name || record.request.company.ticker : currentStudy ? studyName(currentStudy) : t('估值研究工作台', 'Valuation research workspace')}</h1><p>{record ? `${record.request.valuation_date} · ${record.request.company.currency} · ${record.request.mode === 'demo' ? t('合成数据演示', 'Synthetic data demo') : t('基于已提交资料', 'Based on submitted data')} · v${record.revision}` : t('与你的 Agent 一起，从资料走向有据可查的研究。', 'From source material to evidence-backed research, with your Agent.')}</p></div><div className="heading-actions">{record ? <Status status={record.status} t={t}/> : <span className="tag">{currentStudy ? t('已自动保存', 'Saved automatically') : t('新研究', 'New study')}</span>}{record?.result && <><button className="button secondary export-button" onClick={() => downloadRun(record.run_id, 'xlsx').catch(err => setError(err.message))}>Excel</button><button className="button secondary export-button" onClick={() => downloadRun(record.run_id, 'pdf').catch(err => setError(err.message))}>PDF</button><button className="button secondary export-button" onClick={exportResult} title={t('导出 JSON 复算包', 'Export JSON research package')}><Icon name="download" size={16}/>JSON</button></>}</div></div><ErrorNotice message={error || research.error} onDismiss={() => { setError(''); research.refresh() }}/>{health === 'offline' && <div className="offline-notice"><Icon name="link" size={16}/><span>{t('无法连接估值服务。请启动后端；你的当前配置会保留。', 'Cannot reach the valuation service. Start the backend; your current selections are preserved.')}</span><button onClick={syncRuns}>{t('重试', 'Retry')}</button></div>}
    {!runId && !showWizard ? <ResearchDesk key={`research-${draftVersion}`} language={language} t={t} connection={session} initialCompany={researchSeed} initialResearchId={researchId} refreshToken={dataRefresh} onHistoryChange={setResearchStudies} onSessionChange={setResearchId} onConnectionLost={() => setSession(null)} onConnect={() => setModal('connection')} onDataServices={() => setModal('data-services')} onWizard={() => setShowWizard(true)} onValuation={openValuation} onBusyChange={setSending} onRestoreLanguage={chooseLanguage}/> : view === 'workspace' ? <div className="workspace-grid">{conversation}<Inspector record={record} events={events} artifacts={research.artifacts} t={t}/></div> : <div className="report-layout"><div className="report-surface">{view === 'results' ? <Results record={record} t={t}/> : <Evidence record={record} events={events} artifacts={research.artifacts} t={t}/>}</div><details className="report-conversation"><summary>{t('继续讨论 / 修改假设', 'Discuss / revise assumptions')}</summary>{conversation}</details></div>}
  </main></div>
  {modal === 'connection' && <ConnectionModal t={t} session={session} onClose={closeModal} onConnect={connect} onDisconnect={async () => { try { await api(`/api/model-sessions/${session.session_id}`, { method: 'DELETE' }); setSession(null); setModal(null) } catch (err) { setError(err.message) } }}/>} {modal === 'data-services' && researchId && <DataServicesModal t={t} researchId={researchId} status={dataServices} onClose={closeModal} onConnect={connectDataServices}/>} {modal === 'review' && record && <ReviewModal {...{record,t,submitting}} onClose={closeModal} onSubmit={revise}/>} {modal === 'history' && <Modal title={t('研究历史', 'Study history')} onClose={closeModal} className="wide-modal"><input className="history-search" aria-label={t('搜索研究历史', 'Search study history')} value={historyQuery} onChange={e => setHistoryQuery(e.target.value)} placeholder={t('按公司、任务名称或代码搜索', 'Search company, title or ticker')}/>{allStudies.length ? <div className="history-table">{allStudies.filter(run => `${studyName(run)} ${run.request?.company?.ticker || run.draft?.ticker || ''}`.toLowerCase().includes(historyQuery.toLowerCase())).map(run => <button key={run.run_id || run.session_id} onClick={() => selectStudy(run)} disabled={sending || submitting}><span><b>{studyName(run)}</b><small>{run.session_id ? new Date(run.updated_at).toLocaleDateString() : run.request.valuation_date} · {studyMode(run)} · v{run.revision}</small></span>{run.session_id ? <span className="tag">{run.status === 'waiting_confirmation' ? t('待确认', 'Your choice') : t('资料研究', 'Research')}</span> : <Status status={run.status} t={t}/>}<Icon name="chevron" size={16}/></button>)}</div> : <Empty title={t('还没有研究任务', 'No studies yet')}>{t('创建研究后，记录会自动保留。', 'New studies are saved automatically.')}</Empty>}</Modal>}
  </div>
}

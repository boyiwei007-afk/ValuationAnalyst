...
当前交付范围：已实现资料研究 Agent、文件解析与候选确认、长对话、恢复交互、审计、Tushare A股点时取数、Tavily联网搜索、研究候选自动提交、生产同业筛选及 JSON/HTML/PDF/Excel 导出；非金融行业金融小组模型已经接入正式估值工作流。扫描件OCR、多业务分部预测和金融小组金标准案例对账仍需继续完成。
...
| 资料来源 | 选择联网获取或自行上传 | 决定后续是否允许调用 Tushare/Tavily；上传模式不会静默改用在线数据 |

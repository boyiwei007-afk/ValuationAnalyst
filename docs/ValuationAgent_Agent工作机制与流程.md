...
当前交付范围：已实现资料研究 Agent、文件解析与候选确认、长对话、恢复交互、审计、Tushare A股点时取数、巨潮资讯官方年报目录直连、Tavily联网搜索与公开网页正文读取、官方年报 PDF 下载与页级溯源、原始科目确定性推导及财务勾稽、研究候选自动提交、生产同业筛选及 JSON/HTML/PDF/Excel 导出；非金融行业金融小组模型已经接入正式估值工作流。扫描件OCR、多业务分部预测和金融小组金标准案例对账仍需继续完成。
...
- **用户**：确认研究范围、候选事实、重要假设和歧义处理方式。需要决策时，LLM 根据当前上下文生成 2—3 个可执行选项；CLI 与 Web 在末尾统一提供 `Chat`，用于输入自己的判断或修改要求。
...
flowchart TD
    ...
    N --> O[对话 / 可视化 / JSON / HTML / PDF / Excel]
```
...
| 历史财务资料 | 年报 PDF、DOCX、XLSX、CSV/TSV、HTML、JSON、文本 | 解析为可定位原文块，再由 LLM按需读取和映射；公式不执行，扫描件明确要求 OCR |
...
| 资料来源 | 选择 Tushare 结构化取数 + 联网研究、联网检索官方年报，或自行上传 | 年报优先按证券代码和年份直连巨潮官方公告目录，无需搜索 Key；Tavily 负责补充检索并可安全下载公开 PDF/HTML/文本正文，搜索摘要不能直接成为财务事实，上传模式不会静默改用在线数据 |
| 持续估值目标 | “开始估值”“提交正式估值” | 将目标保存为会话状态；不完整时由 LLM 继续检索与提取，确认后自动恢复，准备完成后自动交接正式流水线 |
...
长期记忆具有稳定 `key`，同一事项变化时覆盖原键，并记录来源消息 ID。系统提示词要求模型不要保存：
...
程序会拒绝匹配已配置凭证模式的记忆，但“是否为财务数值、是否准确反映用户意图”的语义判断目前仍依赖模型遵守提示词，不是硬性校验。记忆不会自动写入已确认财务字段；财务数字必须进入独立的候选事实与用户确认流程。Web 的“上下文”页签和 CLI 的 `/memory` 可以查看当前长期记忆。

当事实、文件或记忆超出上下文预算，摘要会显示省略数量；Agent 可通过 `inspect_context` 按类别、关键词或来源 ID 分页查回完整记录，包括早期用户原话。长对话无需在每轮重发全部资料。

### 4.4 LLM 工具循环

LLM 每一步必须调用一个已注册工具。自由文本不能直接修改正式状态或产生正式估值。当前基础工具包括：

- `inspect_context`：读取有界研究摘要，或分页检索事实、记忆、文件清单和早期用户原话；
...
- HTTP 429/502/503/504：供应商层有限重试，仍失败则进入恢复交互；
- 连接异常或超时：保留会话并进入恢复交互，由用户选择重试或重新配置；
...
用户可以选择末尾的 `Chat` 输入自己的处理方式。重试使用原请求和已保存状态，不要求用户重复描述，也不会重复确认已经采纳的数据。
...
正式金融小组模型、A股在线取数、联网搜索、研究转估值和Excel/PDF报告已经按接口接入。扫描PDF OCR、多业务分部预测和完整三表联动仍未接入；这些能力会明确显示限制，不会由LLM模拟执行。
